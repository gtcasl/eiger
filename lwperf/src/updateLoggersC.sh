...
sed -e 's/[ \t]//g' \
 -e 's/.*PERFLOG(\(\w*\)[ ,]*/\tvoid init\1(formatter<PERFBACKEND> *c){/g' \
...
grep -h PERFLOG $cfiles |sed -e 's/.*PERFLOG(\(\w*\).*/case ENUMXXX: { PERF::Log(\1, "\1")->start(); } break; /g' |awk '
BEGIN { count=1 }
{
sub("ENUMXXX",count)
count++
...
grep -h PERFLOG $cfiles |sed -e 's/.*PERFLOG(\(\w*\).*/case ENUMXXX: { PERF::Log(\1, "\1")->stop(); } break; /g' |awk '
BEGIN { count=1 }
{
sub("ENUMXXX",count)
count++
