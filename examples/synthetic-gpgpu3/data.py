...
#
#
def load(classic = True):
	"""
	Constructs a cube of data consisting of:
	
	Inputs = [ static_metrics   dynamic_metrics  estimable_metrics  machine_metrics ]
	Outputs = [ C1060; 280GX ; 8600GS ]
	Predict = [ 8800GTX ]
	"""
	programs = np.array( ['cp', 'mri-fhd', 'mri-q', 'pns2000', 'rpes', 'sad', 'tpacf', 
		'nbody256', 'nbody512', 'nbody1024', 'nbody2048', 'nbody4096', 'pns250', 
		'pns500', 'pns1000', 'pns1500', 'hotspot1', 'hotspot2', 'hotspot3', 
		'hotspot4', 'hotspot5', 'hotspot6', 'hotspot7', 'lu32', 'lu1024'] )

	cpu_machines = np.array(['nehalem', 'atom', 'phenom'])
	gpu_machines = np.array(['8800GTX', 'C1060', '280GTX', '8600GS'])
	machines = np.concatenate((cpu_machines, gpu_machines))

	static_metrics = np.array(['Extent of Memory', 'Context Switches', 'Live Registers', 
	...
	estimatable_metrics = np.array(['Activity Factor', 'Memory Intensity', 
		'Memory Efficiency', 'Memory Sharing', 'SIMD', 'MIMD']),
	dynamic_metrics = np.array(['Dynamic Integer arithmetic', 'Dynamic Integer logical',
	...
	machine_metrics = np.array(['Clock Frequency', 'Hardware Threads Per Core', 'Cores',
	...
	deterministic_metrics = [static_data, dynamic_data, estimatable_data]
	...
	metric_names = static_metrics + dynamic_metrics + estimatable_metrics + machine_metrics
	
	collect = [ '8600GS', '280GTX', 'C1060' ]

	cube = []
	output = []
	profileMachine = []
	profileMachineSelector = []
	for i, gpu in enumerate(collect):
		profileMachine.append(gpu_machine_data[gpu])
		for app in applications:
			row = []
			for metric in deterministic_metrics:
				row += metric[app]
			#row += gpu_machine_data[gpu]
			cube.append(row)
			output.append([ times[gpu][app], ])	
			profileMachineSelector.append(i)
	trainingData = dc.DataCollection(np.matrix(cube), np.matrix(profileMachine), np.matrix(output), 
		applications, np.matrix([[s, 0, 0] for s in profileMachineSelector]), None)
	
	experimentalGpu = ['8800GTX',]
	experimental = []
	experimentalOutput = []
	experimentalMachine = []
	for gpu in experimentalGpu:
		experimentalMachine.append(gpu_machine_data[gpu])
		for app in applications:
			row = []
			for metric in deterministic_metrics:
				row += metric[app]
			...
			experimentalOutput.append([ times[gpu][app], ])	
	experimentalData = dc.DataCollection(np.matrix(experimental), np.matrix(experimentalMachine), 
		np.matrix(experimentalOutput), applications, np.matrix([[0,0,0] for a in applications]), None)
	
	if classic:
		metrics = (metric_names, machine_metrics)
		profile = (np.matrix(cube), np.matrix(profileMachine), np.matrix(output), profileMachineSelector)
		experimental = (np.matrix(experimental), np.matrix(profileMachine), np.matrix(experimentalOutput))
		return dc.DataCube(metrics, profile, experimental, applications)
	else:		
		return (trainingData, experimentalData)
	
	#return (np.matrix(cube), np.matrix(output), metric_names, \
	#	np.matrix(experimental), np.matrix(experimentalOutput), applications)
