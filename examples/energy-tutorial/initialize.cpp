...

{
  ...
  *xexact = new double[local_nrow];
  ...
  PERFLOGKEEP(generate_matrix,ID(nx),ID(ny),ID(nz));
  ...
  PERFSTOPKEEP(generate_matrix,ID(nx),ID(ny),ID(nz));
#ifdef USING_SSTMAC
  SSTMAC_compute_loop3(0, nx, 0, ny, 0, nz, 5 + 4.4 * 27/2);
#endif
...
make_local_matrix(HPC_Sparse_Matrix * A)
{
  ...
  int countit = 0;
  PERFLOGKEEP(make_local_matrixA,IN(local_nrow),IN(total_nnz), IN(num_external) );
  ...
  SSTMAC_compute_loop(0, countit, 3.9*.11);
#endif
  ...
  PERFSTOPKEEP(make_local_matrixA, IN(local_nrow), IN(total_nnz), IN(num_external) );
  ...
  int length = 1;
  PERFLOG(make_local_matrixB, IN(local_nrow), IN(total_nnz), IN(num_external) );
...
#ifdef USING_SSTMAC
  SSTMAC_compute_loop(0, countit, 1.9*.27);
#endif
  ...
  PERFSTOP(make_local_matrixB, IN(local_nrow), IN(total_nnz), IN(num_external) );
#ifdef USING_SSTMAC
